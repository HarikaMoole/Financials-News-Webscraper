...
def upload_to_s3(file_name, bucket, object_name=None):
    ...
    try:
        ...
        logging.info(f"File {file_name} uploaded to S3 bucket {bucket}")
        return True
        ...
        try:
            ...
            for link in soup.find_all('a'):
                if isinstance(link.string, str) and len(link.string) > 50:
                    data.append({"Headline": link.string.strip()})
...
def lambda_handler(event, context):
    try:
        ...
        # Check if the file was created and log the file size
        if os.path.exists(file_name):
            file_size = os.path.getsize(file_name)
            logging.info(f"File created successfully at {file_name}, size: {file_size} bytes")
        else:
            logging.error(f"Failed to create file at {file_name}")
            return {
                'statusCode': 500,
                'body': 'Failed to create CSV file.'
            }
